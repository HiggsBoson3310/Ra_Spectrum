...
!		nljmax == number of different partial waves orbitals 	
	...
	real*4 time(2)
        ...
        logical, parameter :: expth_flag = .true.
...
c       From NIST table
	...
	dimension prevtao(nopmax),smtao(nopmax),npistao(nopmax)
...
cgcg	write(6,*) 'time',time
	...
	open(11, file='finalJ.bin', status='old')
	open(10, file='initJ.bin', status='old')
	read(10,*) jint
	read(11,*) jfin
	close(10)
	close(11)
...
c   the potentials.
c	eI = -3.240880722315326D-01
c	e0 = eI-0.5d0/24.d0**2
c	ef = eI-0.5d0/46.d0**2
c	e0+3.05040d-03
c	ne = 1200
...
c -- Begin main energy loop
	...
	open(unit=27, file="time_dels_jjstream_jf"
	 ...
	 open(unit=227, file="time_dels_sep_jjstream_jf"
     1   //char(jt+48)//".dat",status="unknown")
	 open(unit=29, file="betas_jjstream_jf"
     1   //char(jt+48)//".dat",status="unknown")
	open(unit=28, file="kmatrix_jjstream_jf"
     1   //char(jt+48)//".dat", status="unknown")
 6038 format(16(3x,e20.8))
 6138 format(92(3x,e20.8))
 6238 format(50000(3x,e20.8))
	rewind(27)
	do 9999 ie=1,ne
	   e = e0 + (ie-1)*he
...
c	write(6,*) 'orthogonality test done'
	   ...
	   do 620 iblk=1,nblock
		 ...
		 isum=isum+1
		 if(k.le.ncc .or. isum.ne.1) go to 618
...
cc!		    	if (ie.eq.1) then
c					print *, "For final J ", jt
c					print *, "   Channel ", ic, " reset:"
c					print *, "     old: ", en(n1f(k))
c					print *, "     now: ", expth(ll)
cc!		    	endif
		else
...
3123	format(5(1x,1pd11.4))
  618         continue
  620      continue
	write(29,6123) e, beta
 6123 format(15(f13.8,3x))
c	write(6,*) 'ic,isum=',ic,isum
 
	   if(ic.ne.nchan) stop 'error in block indexing'
...
c   writing K matrix values.
	if(jt.eq.0) then
		write(6, *)"Saving k mat for j 0"
		write(28,6038) e, ((xk(i,j), j=i, nchan), i=1, nchan)
	else if(jt.eq.1) then
		write(6, *)"Saving k mat for j 1"
		write(28,6138) e, ((xk(i,j), j=i, nchan), i=1, nchan)
	else 
		write(6,*)"Saving k mat for J other than 0 or 1"
		write(28,6238) e, ((xk(i,j), j=i, nchan), i=1, nchan)
	endif
...
c      epsexp(2)=10.d0
chg
           eOpenCutoff=0.00d0!-0.03d0*0.0d0
...
c!djh 		write(35,*) eFinalCm,suml,sumv
 		    write(35,*) (e-e0)*2.1947e+5,suml,sumv
			write(65,*) dsqrt(-0.5/(e-eI)),suml,sumv
            write(31,*) e, suml
            write(32,*) e, sumv
...
9010	continue
 ...
 9040	 continue
	 ...
	 if (ii/=nopen) then
		...
		write(155,185) dsqrt(-0.5/(e-eI)), partiall(1:nopen)
	    write(136,185) (e-e0)*2.1947e+5,partialv(1:nopen)
		write(156,185) dsqrt(-0.5/(e-eI)), partialv(1:nopen)
	 endif
...
c where is the derivative to get Q?	
	...
	write(20,8892) e, (tao(irh),irh=1,nopen)
         sumtao=0.d0
		 smtao = 0.0d0
         DO irh=1,nopen
		 ...
		 ELSE
			...
			ENDIF
			do irh=1,nopen
				if(dabs(smtao(irh)-prevtao(irh)).gt.0.5) THEN
					npistao(irh) = npistao(irh)+1
				endif
			enddo
		 ENDIF
		 sumtaoprev = sumtao
		 ...
		 WRITE(226,*) e, smtao(1:nopen)
		 WRITE(227,*) e, smtao(1:nopen)+npistao(1:nopen)
