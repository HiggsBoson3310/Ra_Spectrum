...
!		nljmax == number of different partial waves orbitals
	...
	open(unit=55, file='finalJ.bin', status='old')
	read(55,*) jin
	read(55,*) iod
	close(55)
c ***** input pen basis information
       open(unit=5,file='RaFinalStateJ'//char(jin+48)//'.dat'
     1       ,status='old')
	read(5,*) ipar,jtrue,init
...
c ***** Input r12 integrals
	...
	open(unit=12,file='final12.dat',status='old')
	rewind(12)
	read(12,*) ntot
...
ccc           write(6,*) i,j,olap(i,j)
 ...
 7711   format(5(1pd14.7))
	...
	if(init.eq.1) then
	  ...
	  rewind(16)
	  write(16,*) ipar,jtrue,ncc,ibclos
	  write(16,789) (n1(kk),n2(kk),kk=1,ncc)
  789     format(8(2i4,2x))
	write(16,789) (l1blk(i),l2blk(i),j1blk(i),j2blk(i),i=1,ibclos)
	  write(16,789) (kkstart(i),kkend(i),i=1,ibclos)
	  write(16,4) eval(1)
	  write(16,4) (vec(i,1),i=1,ncc)
	  close(16)
	  stop
	else
...
c --------  and transform to the eigenrepresentation
  ...
  860   continue
	...
	write(6,*) 'maximum asymmetry in HL is ',diff
        write(6,*) 'ncc,noo,ntot',ncc,noo,ntot
  ...
  952	continue
 1000   continue
        write(6,*) 'transformation completed'
	...
	write(6,*) 'output to unit 15 finished'
